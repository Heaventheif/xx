...
export function createListenMqtt(deps) {
  const {
    WebSocket,
    mqtt,
    HttpsProxyAgent,
  ...
  return function listenMqtt(defaultFuncs, api, ctx, globalCallback) {
    ...
    let host;
    if (ctx.mqttEndpoint) host = `${ctx.mqttEndpoint}&sid=${sessionID}&cid=${ctx.clientId}`;else if (ctx.region) host = `wss://edge-chat.facebook.com/chat?region=${ctx.region.toLowerCase()}&sid=${sessionID}&cid=${ctx.clientId}`;else host = `wss://edge-chat.facebook.com/chat?sid=${sessionID}&cid=${ctx.clientId}`;
    const options = {
    ...
    mqttClient.on("connect", function () {
      ...
      mqttClient.subscribe(topicList, subErr => {
        ...
        mqttClient.publish(syncTopic, JSON.stringify(queue), {
          qos: 1,
          retain: false
        });
        ...
        ctx.tmsWait = function () {
          ...
          if (ctx.globalOptions.emitReady) globalCallback({
            type: "ready",
            error: null
          });
          delete ctx.tmsWait;
      ...
      try {
        ...
        if (jsonMessage.type === "jewel_requests_add") {
          ...
          if (tasks && tasks instanceof Map && tasks.has(reqID)) {
            ...
            if (taskRespData == null) taskCallback("error", null);else taskCallback(null, Object.assign({
              type: taskType,
              reqID
            }, taskRespData));
          }
...
export default createListenMqtt;