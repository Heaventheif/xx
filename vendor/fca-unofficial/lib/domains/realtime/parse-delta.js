...
function emitThreadInfoEvent(ctx, globalCallback, payload) {
  ...
  if (payload && payload.type === "event" && payload.threadID != null && typeof ctx._syncThreadInfoFromEvent === "function") {
    try {
...
export function createParseDelta(deps) {
  const {
    parseAndCheckLogin
  } = deps;
  return function parseDelta(defaultFuncs, api, ctx, globalCallback, {
    delta
  }) {
              ...
              if (d.deltaMessageReply.repliedToMessage) {
                ...
                return defaultFuncs.post("https://www.facebook.com/api/graphqlbatch/", ctx.jar, {
                  ...
                  callbackToReturn.messageReply = {
                    ...
                    attachments: fetchData.message.blob_attachment.map(att => _formatAttachment({
                      blob_attachment: att
                    }, undefined)),
...
export default createParseDelta;