  ...
  switch (data.__typename) {
        ...
        case "CHANGE_THREAD_ICON":
          {
            ...
            return {
              ...
              logMessageData: {
                thread_icon_url: codepoint ? `https://static.xx.fbcdn.net/images/emoji.php/v9/t3c/1/16/${codepoint}.png` : null,
            ...
            return {
              ...
              logMessageData: {
                question_json: JSON.stringify({
                  ...
                  options: Array.isArray(question?.options?.nodes) ? question.options.nodes.map(option => ({
                    id: option.id,
                    text: option.text,
                    total_count: Array.isArray(option.voters?.nodes) ? option.voters.nodes.length : 0,
                    viewer_has_voted: option.viewer_has_voted,
                    voters: Array.isArray(option.voters?.nodes) ? option.voters.nodes.map(voter => voter.id) : []
                  })) : []
                }),
    ...
    case "UserMessage":
      return {
        ...
        attachments: Array.isArray(data.blob_attachments) && data.blob_attachments.length > 0 ? formatBlobAttachments(data.blob_attachments) : formatExtensibleAttachment(data.extensible_attachment),
...
export function createGetMessageQuery(deps) {
  const {
    defaultFuncs,
    ctx,
    logError
  ...
  return function getMessage(threadID, messageID, callback) {
    ...
    if (!threadID || !messageID) {
      cb({
        error: "getMessage: need threadID and messageID"
      });
      return promise;
    }
