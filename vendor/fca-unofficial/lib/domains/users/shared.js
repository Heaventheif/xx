...
export function toJSONMaybe(input) {
  if (!input) {
    return null;
  }
...
export function usernameFromUrl(raw) {
  if (!raw) {
    return null;
  }
  try {
...
export function normalizePrimaryActor(actor) {
  if (!actor) {
    return null;
  }
  return {
...
export function normalizeCometUser(user) {
  if (!user) {
    return null;
  }
...
export function mergeUserEntry(primary, fallback) {
  if (!primary && !fallback) {
    return null;
  }
  const a = primary || {};
  ...
  return {
    ...
    isMessageBlockedByViewer: typeof a.isMessageBlockedByViewer === "boolean" ? a.isMessageBlockedByViewer : Boolean(b.isMessageBlockedByViewer),
    workInfo: a.workInfo || b.workInfo || null,
    messengerStatus: a.messengerStatus || b.messengerStatus || null
  };
}
export function toUserInfoEntry(raw, idFallback) {
  return {
...
export function formatUserIdEntry(data) {
  return {
