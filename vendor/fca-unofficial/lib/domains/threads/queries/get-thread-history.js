    ...
    switch (message.__typename) {
      case "UserMessage":
        {
          ...
          return {
            type: "message",
            attachments: maybeStickerAttachment ? maybeStickerAttachment : message.blob_attachments && message.blob_attachments.length > 0 ? message.blob_attachments.map(formatAttachmentsGraphQLResponse) : message.extensible_attachment ? [formatExtensibleAttachment(message.extensible_attachment)] : [],
            ...
            messageReactions: message.message_reactions ? message.message_reactions.map(formatReactionsGraphQL) : null,
      ...
      case "ThreadImageMessage":
        return {
          ...
          logMessageData: {
            thread_icon: message.image_with_metadata ? message.image_with_metadata.preview.uri : null
          }
        };
      ...
      default:
        return {
          error: `Don't know about message type ${message.__typename}`
        };
    }
  });
}
export function createGetThreadHistoryQuery(deps) {
  const {
    defaultFuncs,
    ctx,
    logError
