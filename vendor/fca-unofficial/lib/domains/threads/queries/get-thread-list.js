...
function mapNicknames(customizationInfo) {
  return customizationInfo && Array.isArray(customizationInfo.participant_customizations) ? customizationInfo.participant_customizations.map(user => ({
...
function formatThreadList(data, logError) {
  return data.map(thread => {
    ...
    return {
      threadID: thread.thread_key ? formatID(String(thread.thread_key.thread_fbid || thread.thread_key.other_user_id || "")) ?? null : null,
      ...
      snippetSender: lastMessageNode ? formatID(String(lastMessageNode.message_sender?.messaging_actor?.id || "")) ?? null : null,
...
export function createGetThreadListQuery(deps) {
  const {
    defaultFuncs,
    ctx,
    logError
  ...
  return function getThreadList(limit, timestamp, tags, callback) {
    ...
    (0, graphql_1.postGraphqlBatch)({
      ...
      if (!Array.isArray(resData) || resData.length === 0) {
        throw {
          error: "getThreadList: Invalid response data"
        };
      }
      const last = resData[resData.length - 1];
      if (!last || typeof last !== "object") {
        throw {
          error: "getThreadList: Invalid response tail"
        };
      }
      if (last.error_results > 0) {
        ...
        throw {
          error: "getThreadList: error_results > 0"
        };
      }
      ...
      if (!Array.isArray(nodes)) {
        throw {
          error: "getThreadList: Invalid payload structure"
        };
      }
