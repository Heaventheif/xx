...
export async function applyThreadInfoRealtimeEvent(Thread, threadID, ev, log, api) {
  ...
  try {
    const row = await Thread.findOne({
      where: {
        threadID
      }
    });
...
export function attachThreadInfoRealtimeSync(ctx, models, logger, api) {
