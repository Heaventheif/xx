 ...
 * New command — fills a gap in the bot's introspection tools.
 */

const _threadStats = new Map();   // threadID → { msgCount, users: Map<uid, count>, commands: Map<name, count> }
const _globalStats = {
  totalMessages: 0,
  totalCommands: 0,
  startedAt: Date.now(),
...
// Record a message event — called from onChat so it's passive.
export function recordMessage(threadID, senderID, commandName) {
  ...
  if (!t) {
    t = { msgCount: 0, users: new Map(), commands: new Map() };
...
export default {
  ...
  // Passive listener — records every message silently.
  onChat: async ({ event }) => {
