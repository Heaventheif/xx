...
function getTypingDuration(text) {
  ...
  if (!text) return 0;
  // وسيط 350ms مع انحراف 0.35 — يُعطي نطاقاً واقعياً بين ~150ms و~700ms
  return Math.min(nextLogNormal(350, 0.35), 700);
}
...
export const $plugin = {
  ...
  meta: { category: 'utils', path: 'src/utils/bot-enhancer.js' },
  setup(_ctx) {
    // provides: enhanceBot
  },
};
