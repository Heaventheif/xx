...
import { getHfBaseOrNull, getInternalToken } from "./hfClient";
...
// Provider 1: HF-space bridge — this backend already falls back to FerDev
// server-side (fallback_ferdev: true), so it's tried first.
...
// Provider 2: FerDev API called directly (used if the HF bridge is unavailable).
...
// Try the HF bridge first, then FerDev directly, returning the first successful image list.
...
// Download a list of image URLs to disk with a concurrency limit.
