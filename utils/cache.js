...
function get(key) {
  ...
  if (!entry) {
    // Use PerformanceManager's own cache miss path (get() on a missing key)
    // rather than mutating .metrics directly (which is an internal detail).
  ...
  // PerformanceManager records a hit only when get() finds the key in its own
  // internal _cache — we cannot hook its counter from outside without breaking
  // encapsulation.  Bridge is best-effort: we just skip the metric update here
  // rather than directly touching _pm.metrics (an internal implementation detail).
  return entry.value;
}
