...
function parseAndCheckLogin(ctx, defaultFuncs) {
    return function (res) {
        ...
        // undefined, so this threw "Empty response body" on every single
        // call. That silently broke OldMessage.js (used for DM attachments,
        // and as the fallback whenever an MQTT send fails).
        var status = res && res.status !== undefined ? res.status : (res && res.statusCode);
        if (status >= 500 && status < 600) {
            throw { error: "Facebook internal server error: " + status, res };
        }
        if (status === 404 || status === 410) {
            throw { error: "Endpoint not found: " + status, res };
        }
        var rawBody = res && res.data !== undefined ? res.data : (res && res.body);
        if (rawBody === undefined || rawBody === null || rawBody === "") throw { error: "Empty response body", res };
