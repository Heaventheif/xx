...
/**
 * FCA-NX E2EE Bridge — Signal Protocol + Noise WebSocket
 *
 * Uses FCA-NX's own native E2EE engine bundled at ./vendor/fb-e2ee.cjs.
 * No external npm package needed — all E2EE code is owned by FCA-NX.
 *
...
function loadFBClient() {
    try {
        ...
        throw new Error(
            "FCA-NX E2EE engine failed to load.\n" +
...
class E2EEBridge {
    ...
    async _doConnect(deviceStorePath, userId) {
        ...
        // Default to .fca-nx/e2ee_device.json in the user's project directory
        if (!deviceStorePath) {
            deviceStorePath = path.join(process.cwd(), ".fca-nx", "e2ee_device.json");
        }
        ...
        // Re-use the already-loaded session from FCA-NX's cookie jar
        ...
        // Build the FCA-NX adapter that replaces fb-messenger-e2ee's internal
        ...
        // skips the real Facebook login, returning FCA-NX's session instead.
        global._fcanxE2EEAdapter = _fcanxAdapter;

        logger.info("E2EE", "Bootstrapping auth via vendored E2EE engine (FCA-NX session)...");
        let resolvedUserId;
        try {
     ...
     * Attachments on non-E2EE threads fall back to api.sendMessage (FCA-NX's own).
     */
