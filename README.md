...
### سياسة التشغيل الحالية

يعمل البوت داخل **المجموعات فقط**. يتم تجاهل الرسائل الخاصة وطلبات الرسائل المعلقة دون الرد عليها أو قبولها. يقوم مجدد الجلسة بطلبات تحقق دورية محدودة، ثم يحفظ فقط cookies التي يعيدها الـ cookie jar الفعلي؛ لا يغيّر التطبيق تاريخ انتهاء cookie ولا يخترع مدة صلاحية جديدة. عند رفض AppState يجب إنشاء جلسة جديدة من جهاز موثوق وتحديث الإعدادات، وليس تعديل تاريخ الانتهاء محلياً.
...
### أوامر Xalman المضافة

أضيفت أربعة أوامر داخل `src/commands/xalman/`، وتستخدم فقط نقاط Xalman التي تم اختبارها وإرجاعها HTTP 200:

| الأمر | البدائل | الوظيفة |
|---|---|---|
| `xgpt` | `xai`, `سؤال`, `سوال` | إجابة نصية سريعة عبر GPT |
| `يوتيوب` | `ytsearch`, `يوت` | البحث عن فيديوهات يوتيوب وإظهار الروابط |
| `سبوتيفاي` | `spotify`, `spsearch` | البحث عن الأغاني والفنانين وإظهار روابط Spotify |
| `كلمات` | `lyrics`, `كلمات_اغنية` | جلب كلمات أغنية مع اختصار النص الطويل |

لم تُضف أي أوامر خاصة بالأنمي أو الطقس أو المحتوى الإباحي. تعتمد هذه الأوامر على الاتصال الخارجي بـ `https://xalman-apis.vercel.app/api`، ولذلك تعرض رسالة مناسبة عند انتهاء المهلة أو فشل الخدمة.
